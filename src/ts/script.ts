...
const btn_co_from = document.querySelector(
  "button#co_from"
) as HTMLButtonElement;
const btn_co_timer = document.querySelector(
  "button#co_timer"
) as HTMLButtonElement;
const btn_co_interval = document.querySelector(
  "button#co_interval"
) as HTMLButtonElement;
const btn_co_range = document.querySelector(
  "button#co_range"
) as HTMLButtonElement;
const btn_co_empty = document.querySelector(
  "button#co_empty"
) as HTMLButtonElement;
const btn_co_iif = document.querySelector("button#co_iif") as HTMLButtonElement;
const btn_co_throwError = document.querySelector(
  "button#co_throwError"
) as HTMLButtonElement;
const btn_co_defer = document.querySelector(
  "button#co_defer"
) as HTMLButtonElement;
const btn_co_range_filter = document.querySelector(
  "button#co_range_filter"
) as HTMLButtonElement;
const btn_co_from_duc = document.querySelector(
  "button#co_from_duc"
) as HTMLButtonElement;
const btn_co_range_first = document.querySelector(
  "button#co_range_first"
) as HTMLButtonElement;
const btn_co_range_last = document.querySelector(
  "button#co_range_last"
) as HTMLButtonElement;
const btn_co_range_single = document.querySelector(
  "button#co_range_single"
) as HTMLButtonElement;
const btn_co_range_single_mono = document.querySelector(
  "button#co_range_single_mono"
) as HTMLButtonElement;
const btn_co_from_single_mono = document.querySelector(
  "button#co_from_single_mono"
) as HTMLButtonElement;
const btn_co_from_single_poly = document.querySelector(
...
function toggleBtnStatusClass(btn: HTMLButtonElement) {
  btn.classList.contains("unsubscribe")
    ...
    ? (btn.classList.remove("create"), btn.classList.add("unsubscribe"))
    : null;
}
...
function subscribe_with_pipeline(
  observableExemple: Observable<any>,
  description: string,
  pipeLine: any,
  btn: HTMLButtonElement
  ...
  observable_range
    ...
    .subscribe((value) => console.log("Filtering renge", value));
...
(function eventPromise() {
  ...
  const p = new Promise((resolve) => {
    ...
    // assigning a data iput event handler
    // it works first and activates the method of promice
    input_1.addEventListener("input", listener);
  });
  // processing onFulfilled data input event
  p.then((event: KeyboardEvent) => {
    const value = (event.target as HTMLInputElement).value;
    console.log("Recursive function promise works:", value);
    // make got hub request
    fetch(`https://api.github.com/search/repositories?q=${value}`)
      .then((response) => response.json())
      .then((parsedInfo) => {
        console.warn(parsedInfo);
      });
    // remove lisner from input
...
const mainSubscription = intervalObs_1.subscribe((x) =>
  console.log("first: " + x)
);
const childSubscription = intervalObs_2.subscribe((x) =>
...
function createObservable() {
  ...
  function operateObservable() {
    if (action === "create") {
      observable_1 = new Observable(function subscriber(
        observer: Observer<string | number>
      ) {
        try {
          let counter = 0;
...
btn_co_throwError.addEventListener(
  "click",
  createObservable_throwError_Instance
);
...
const rengeProcessing_find = observable_range
  .pipe(
    find((value: number) => {
      return value === 6;
    })
...
const fromProcessing_find_2 = from([1, 3, 5, 67, 5, 92])
  .pipe(
    find((value: number) => {
      return value === 11;
    })
...
const rengeProcessing_2 = observable_range.pipe(ignoreElements()).subscribe(
  (nextData) => console.log("rengeProcessing_2:", nextData),
  (errorData) => console.warn(errorData),
  () => console.warn("Completed rengeProcessing_2 !")
);

const rengeProcessingDebounce_interval = observable_range
...
const rengeProcessingDebounce_timer = interval(1000)
  .pipe(debounce(() => timer(999)))
  .subscribe(
    (nextData) =>
      console.log("rengeProcessingDebounce timer:", nextData, Date.now()),
    (errorData) => console.warn("rengeProcessingDebounce timer", errorData),
    () => console.warn("Completed rengeProcessingDebounce timer!")
  );

const intervalProcessingDebouncetTime = interval(1000)
  .pipe(debounceTime(999))
  .subscribe(
    (nextData) =>
      console.log("rengeProcessingDebouncetTime:", nextData, Date.now()),
    (errorData) => console.warn("rengeProcessingDebouncetTime", errorData),
    () => console.warn("Completed rengeProcessingDebouncetTime!")
  );

const timerProcessing_throttleTime = timer(0, 200)
  .pipe(throttleTime(600))
  .subscribe(
    (nextData) =>
      console.log("timerProcessing_throttleTime:", nextData, Date.now()),
    (errorData) => console.warn("timerProcessing_throttleTime", errorData),
    () => console.warn("Completed timerProcessing_throttleTime!")
  );

const timerProcessing_throttle = timer(0, 200)
  .pipe(throttle((ev) => interval(600)))
  .subscribe(
    (nextData) =>
      console.log("timerProcessing_throttle:", nextData, Date.now()),
    (errorData) => console.warn("timerProcessing_throttle", errorData),
    () => console.warn("Completed timerProcessing_throttle!")
  );

const timerProcessing_audit = timer(0, 200)
  .pipe(audit((ev) => interval(600)))
  .subscribe(
...
const fromProcessing_skip = from([1, 3, 5, 8, 13, 30])
  .pipe(skip(3))
  .subscribe(
...
const intervalProcessing_skipUntil = interval(1000)
  .pipe(skipUntil(timer(1500, 1000)))
  .subscribe(
...
const intervalProcessing_take = interval(1000)
  .pipe(take(5))
  .subscribe(
...
const intervalProcessing_takeUntil = interval(500)
  .pipe(takeUntil(timer(3600)))
  .subscribe(
...
const intervalProcessing_takeWhile = interval(500)
  .pipe(
...
combineLatestExemple.subscribe({
  next: (value: Array<number>) =>
    console.log("CombineLatestExemple next:", value),
  complete: () => console.warn("Complete: combineLatestExemple"),
  error: (error) => console.log("Error", error),
});

const combineLatestProjectExemple = combineLatest(
  timer_1,
  timer_2,
  timer_3
).pipe(
...
combineLatestProjectExemple.subscribe({
  next: (value: number) =>
    console.log("combineLatestProjectExemple next:", value),
...
const startWithExemple = of(
  "Main observable start",
  "Main observable works",
  "Main observable finsh"
).pipe(startWith("Before starting observsble", "Lets start"));
...
const withLatestFromExemple = timer_1.pipe(withLatestFrom(timer_3));
...
const pairwiseExemple = timer(500, 250).pipe(take(8), pairwise());
...
const racer_1 = timer(5, 100).pipe(
  take(2),
  map((value) => "racer1")
);
const racer_2 = timer(0, 100).pipe(
  take(2),
  map((value) => "racer2")
);
...
const pulsar = timer(100, 250).pipe(mapTo(1), takeUntil(interval(1250)));

pulsar.pipe(reduce((ac, value) => ac + value, 0)).subscribe({
...
pulsar.pipe(scan((ac, value) => ac + value, 0)).subscribe({
...
const mapExemple = of(20, 16, 19, 22, 18, 24)
  .pipe(
    map((value) => {
...
const catchErrorExemple = of(1, 2, 3, 4, 5).pipe(
  map((n) => {
    if (n === 2) {
      throw "Reach 2!";
    }
  ...
  catchError((err) => {
    if (err === "Reach 2!") {
      ...
      throw "Maximum level reached" + err;
    }
  })
);

catchErrorExemple.subscribe({
  next: (value: any) => console.log("catchError:", value),
  complete: () => console.warn("Complete: catchError"),
  error: (error) => console.error("Error catchError:", error),
});
...
const pipeAnalog = (...fns: Function[]) => {
  ...
  return (source: Observable<any>) => {
    return fns.reduce((newSourse, fn) => {
      return fn(newSourse);
    }, source);
  };
};
...
timer(1500, 1000)
  .pipe(operatorCombination_2)
  .subscribe({
    next: (value: any) =>
      console.log("Test operatorCombination operator:", value),
    complete: () => console.warn("Complete: operatorCombination operator"),
    error: (error) =>
      console.error("Error operatorCombination operator:", error),
  });
